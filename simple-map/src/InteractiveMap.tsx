...
const InteractiveMap: React.FC = () => {
  ...
  const routeSegments = useMemo<RouteSegment[]>(() => {
    ...
    return selectedYearRaces.slice(0, -1).map((race, idx) => {
      ...
      const arrowNextPoint = coordinates[Math.min(arrowIndex + 1, coordinates.length - 1)] ?? arrowCoordinates;
      const angleRad = Math.atan2(arrowNextPoint[1] - arrowCoordinates[1], arrowNextPoint[0] - arrowCoordinates[0]);
      const arrowAngle = (angleRad * 180) / Math.PI;
      return {
            ...
            {routeSegments.map((segment) => {
              ...
              return (
                <Marker key={`arrow-${segment.id}`} coordinates={segment.arrowCoordinates}>
                  <g
                    transform={`scale(${invZoom}) rotate(${segment.arrowAngle})`}
